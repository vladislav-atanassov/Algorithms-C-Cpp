...
{
    const T* key = static_cast<const T*>(a);
    const TreeNode<T>* node = static_cast<const TreeNode<T>*>(b);

    if(*key < node->data) return -1;
    if(*key > node->data) return 1;
...
template<class T>
void BinaryTree<T>::deleteFirstByKeyNode(const T& key, TreeNode<T>* root)
{
    
}

template<class T>
void BinaryTree<T>::clear(TreeNode<T>* root) noexcept
{
    if(root != nullptr)
    {
        clear(root->left);
